...
当前默认执行器是真实 LangGraph `StateGraph`。它运行受控、规则优先的节点逻辑，并保留旧版 orchestrator 作为兼容执行器和 parity 测试对象。

项目采用 rule-first + LLM-compose 架构：意图路由、工具选择、权限、风险和副作用边界由确定性代码控制，LLM 主要负责最终客服回复生成。
...
| Agent Runtime | Live LangGraph StateGraph、兼容 orchestrator、Mock LLM、OpenAI-compatible client |
...
评测指标包括 intent accuracy、tool accuracy、tool argument accuracy、missing-slot accuracy、forbidden-tool violation、citation hit rate、groundedness、unsafe blocking、handoff precision、task success 和 latency。CI release gate 使用当前 51 条唯一 JSONL fixtures，不通过时阻断合并；详见 `docs/evaluation.md` 与 `docs/eval_report_mock.md`。
...
- 默认执行器已经是真实 LangGraph runtime；但 Agent 仍是 rule-first + LLM-compose，不应包装成自由规划型 autonomous agent。
- 种子数据为合成数据，主要服务于演示覆盖。

## 路线图

- 增加 live-model shadow eval 与 badcase 趋势对比。
- 将 PostgreSQL 自动建表迁移到 Alembic migrations。
